...
function MemberChat() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      // 2. bookings where I'm the worker with active statuses
      ...
      const otherIds = Array.from(
        new Set((bks ?? []).map((b: any) => b.customer_id).filter(Boolean)),
      ) as string[];
      ...
      const list: Convo[] = otherIds.map((oid) => {
        ...
        const name = nameMap.get(oid) || t("memberHome.customer");
        return {
          otherId: oid,
          name,
      ...
      <div className="px-5 py-4">
        {loading ? (
          ...
          <div className="mt-8 bg-card border border-dashed border-border rounded-3xl p-6 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">{t("chat.noConversationsSubWorker")}</p>
          </div>
        ) : (
