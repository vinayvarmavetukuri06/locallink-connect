  ...
  return (
    <div className="mobile-shell px-5 py-6">
      ...
      <div className="space-y-5">
        ...
        {/* Secondary card — I Provide Services */}
        <Link
          to="/auth/member"
          className="block bg-muted/40 border border-border p-5 rounded-2xl hover:border-muted-foreground/30 active:scale-[0.99] transition-all"
        >
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground mb-0.5">
                {t("auth.chooser.memberLabel")}
              </p>
              <h3 className="font-semibold text-sm font-sans leading-tight">{t("auth.chooser.memberTitle")}</h3>
              <p className="text-xs text-muted-foreground mt-0.5">{t("auth.chooser.memberDesc")}</p>
            </div>
